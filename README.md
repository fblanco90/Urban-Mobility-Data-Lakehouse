...
2.  **Job Queue:**
    *   **Name:** `DuckJobQueue` (or similar)
    *   **Priority:** 1
3.  **Job Definition:**
    *   **Name:** `DuckJobDefinition` (or similar)
...
*   **Connection ID:** `aws_s3_conn` This is a very important step to configure as the code will look only for this ID.
...
*   **Connection ID:** `neon_catalog_conn` This is a very important step to configure as the code will look only for this ID.
*   **Conn Type:** `Postgres`
...
Once both verification dags are succeded, you may execute DAG 1 and after that, DAG 2. Then, you can trigger any 3* DAG.
