...
\begin{document}
	...
	The workflow is organized into a hierarchical structure controlled by a Master Orchestrator, as described below:
	...
	\begin{enumerate} 
		...
		\item \textbf{Gold Generation DAG:} Triggered upon the completion of ingestion, this pipeline aggregates the Silver data to construct the core analytical models (e.g., K-Means Clustering, Gravity Models) in the Gold layer.
		
		\item \textbf{Business Reporting DAGs:} A set of independent, on-demand DAGs designed to query the pre-computed Gold layer. These are isolated from the ELT process to allow ``Transport Experts'' to generate specific reports (e.g., specific Polygon filters or time windows) without re-processing the underlying data.
		
		\item \textbf{Master Orchestrator DAG:} A controller pipeline containing no logic other than the triggering of downstream DAGs. It manages the sequential dependencies between infrastructure setup, data ingestion, and model training, ensuring referential integrity across the Lakehouse.
	\end{enumerate}
	...
	\subsection{Gold Generation (DAG 3)}
	[TODO]
	
	\subsection{Bussines Reporting (DAG 4-5-6)}
	[TODO]
	
	
	\subsection{Master Orchestrator (DAG 7)}
	[TODO]
